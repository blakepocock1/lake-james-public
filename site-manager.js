...
const editableSelector='main h1,main h2,main h3,main h4,main p,main li,main a,main img,footer p,footer span,footer strong,footer img';
...
function applyOverrides(list){if(!Array.isArray(list))return;const nodes=editableNodes();for(const o of list){const el=nodes[o.i];if(!el)continue;if(typeof o.text==='string'&&el.tagName!=='IMG')el.textContent=o.text;if(el.tagName==='A'&&o.href){const u=safeUrl(o.href);if(u)el.setAttribute('href',u)}if(el.tagName==='IMG'&&o.src){const u=safeUrl(o.src);if(u)el.setAttribute('src',u)}}}
try{
 ...
 if(x.page_title)document.title=x.page_title;if(x.meta_description){let m=document.querySelector('meta[name="description"]');if(!m){m=document.createElement('meta');m.name='description';document.head.appendChild(m)}m.content=x.meta_description}
